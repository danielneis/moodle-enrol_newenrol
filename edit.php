...
require_capability('enrol/newenrol:config', $context);

$PAGE->set_url('/enrol/newenrol/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
/** @var enrol_newenrol_plugin $plugin */
$plugin = enrol_get_plugin('newenrol');

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'newenrol', 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_newenrol_edit_form(NULL, array($instance, $plugin, $context));
...
$PAGE->set_title(get_string('pluginname', 'enrol_newenrol'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_newenrol'));
$mform->display();
echo $OUTPUT->footer();
